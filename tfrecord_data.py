...
def input_fn(params):
    ...
    cell_input_size, drug_input_size = params['input_sizes']
    ...
    def _parse_record_fn(raw_record):
        ...
        feature_map = {
            'cell': tf.io.FixedLenFeature([cell_input_size], dtype=tf.float32),
            'drug': tf.io.FixedLenFeature([drug_input_size], dtype=tf.float32),
            'AUC':  tf.io.FixedLenFeature([1], dtype=tf.float32)
        }
        ...
        features = tf.concat([cell, drug], axis=0)
